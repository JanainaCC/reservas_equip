...
if ((empty(strtotime($data_inicio))) || (empty(strtotime($temp_c))) || (empty(strtotime($data_fim))) || (empty(strtotime($temp_f))) || (empty($descricao)) || (empty($cod_item))) {
	...
	$timestamp_data_hoje = explode("-", $data_hoje);
	$timestamp_hora_hoje = explode(":", $hora_hoje);
	$timestamp_hoje=mktime($timestamp_hora_hoje[0],$timestamp_hora_hoje[1],0,$timestamp_data_hoje[1],$timestamp_data_hoje[2],$timestamp_data_hoje[0]);
	...
	if (($time_inicio > $time_fim) || ($time_inicio < $timestamp_hoje)){
		echo "<script type='text/javascript'> alert ('Data da reserva inválida!'); window.location.href='javascript:history.back()'; </script>";
	}else{

		$sql1 = "SELECT * FROM t_reserva WHERE t_item_cod_item = $cod_item and cancela_reserva = 0";
		...
		if ($result1) {
			if (mysqli_num_rows($result1)>0) {
					...
					while($num_rows>0){
						...
						if((($Time_inicio_banco < $time_inicio) AND ($Time_fim_banco < $time_inicio )) OR (($Time_inicio_banco > $time_fim) AND ($Time_fim_banco > $time_fim))){
							...
							if ($result_reservar1) {
								...
								echo "<script type='text/javascript'> alert ('RESERVA REALIZADA COM SUCESSO!'); window.location.href='../minhas_reservas.php'; </script>";
							}else{
								mysqli_close($conexao);
				...
				if ($result_reservar) {
					mysqli_commit($conexao);
					mysqli_close($conexao);
					echo "<script type='text/javascript'> alert ('RESERVA REALIZADA COM SUCESSO!'); window.location.href='javascript:history.back()'; </script>";
				}else{
					mysqli_close($conexao);
					echo "<script type='text/javascript'>alert ('NÃO FOI POSSIVEL REALIZAR RESERVA!'); window.location.href='javascript:history.back()'; </script>";
				}
